...
def init(light_position):
    ...
    glLightfv(GL_LIGHT0, GL_DIFFUSE, (1, 1, 1, 1))  # White diffuse light
    glLightfv(GL_LIGHT0, GL_SPECULAR, (1, 1, 1, 1))  # White specular light
...
def draw_voxel(x, y, z):
    ...
    (-1 + x, -1 + y, 1 + z),  # 6 Back Bottom Left
    (-1 + x, 1 + y, 1 + z)    # 7 Back Top Left
    )
    
    # Define faces using the vertices
    faces = (
    (0, 1, 2, 3),  # Front face
    (4, 5, 7, 6),  # Back face (corrected to maintain CCW order)
    (0, 4, 5, 1),  # Right face
    (2, 6, 7, 3),  # Left face (corrected to maintain CCW order)
    (3, 7, 6, 0),  # Bottom face (corrected to maintain CCW order)
    (1, 5, 4, 2)   # Top face (corrected to maintain CCW order)
    )
    '''
    normals = [
    (0, 0, -1),  # Front face
...
def main():
    ...
    while True:
        ...
        glRotatef(2, 1, 1, 1)  # Optional: Rotate the voxel for better visualization
